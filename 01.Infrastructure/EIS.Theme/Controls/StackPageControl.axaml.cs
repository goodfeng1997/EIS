...
public class StackPageControl : TemplatedControl
{
    public static readonly StyledProperty<object> ContentProperty =
        AvaloniaProperty.Register<StackPageControl, object>(nameof(Content));
    ...
    public static readonly StyledProperty<int> LimitProperty =
        AvaloniaProperty.Register<StackPageControl, int>(nameof(Limit), defaultValue: 5);
    ...
    private static readonly DynamicResourceExtension ColorResource = new("GlobalLowText");
    ...
    private void UpdateContentChanged(object? newVal)
    {
        ...
        if (newVal is IGlobalStackPageTitleProvider stackPageVm)
        {
        ...
        else if (newVal is Control c)
        {
            ...
            {
                model = new StackPageModel(c.Name ?? "", newVal);
                c.AttachedToVisualTree += (_, _) => model.Title = c.Name ?? "";
            }
        }
        else
...
public interface IGlobalStackPageTitleProvider
{
