...
//Table
using CusTableAttribute = EIS.Shared.ExpressionToSql.Attributes.TableAttribute;
using SysTableAttribute = System.ComponentModel.DataAnnotations.Schema.TableAttribute;

//Column
using CusColumnAttribute = EIS.Shared.ExpressionToSql.Attributes.ColumnAttribute;
using SysColumnAttribute = System.ComponentModel.DataAnnotations.Schema.ColumnAttribute;

//Key
using CusKeyAttribute = EIS.Shared.ExpressionToSql.Attributes.KeyAttribute;
...
namespace EIS.Shared.ExpressionToSql.Entry
{
	...
	public class SqlWrapper
    {
        ...
        /// </summary>
        public bool IsEnableNullValue { get; set; }
        ...
        public Dictionary<string, (object data, DataTypeAttribute type)> DataTypedDbParameters
        {
            get
            {
                ...
                var splitSql = this.Replace("NOT IN", "IN")
                                   ...
                                   .ToString()
                                   .Split(' ', ',')
                                   .Select(x => x.Contains(MatchTypeEnum.All, "(", ")")
                                        ? x.Substring("(", ")", false)
                                        : x.Replace("(", "").Replace(")", ""))
                                   .Where(x => !x.Contains("%", "||", "+"))
                                   .ToList();
                ...
                foreach (var parameter in emptyParameters)
                {
                    ...
                    if (field.Contains(MatchTypeEnum.All, "(", ")"))
                        field = field.Substring("(", ")", false);

                    //判断缓存中是否存在DataType
                    if (this._dataTypeDictionary.TryGetValue(field, out var dataType))
                    {
                        //赋值数据类型
        ...
        public List<ColumnInfo> GetPrimaryKey(Type type)
        {
            ...
            foreach (var property in properties)
            {
                ...
                else if (property.GetFirstOrDefaultAttribute<SysKeyAttribute>() is SysKeyAttribute)
                {
