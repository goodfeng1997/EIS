...
namespace EIS.Shared.ExpressionToSql.Expressions
{
    ...
    public class MemberExpressionResolver : BaseExpression<MemberExpression>
    {
        ...
        public override SqlWrapper Where(MemberExpression expression, SqlWrapper sqlWrapper)
        {
            ...
            if (expression != null)
            {
                if (expression.Expression?.NodeType == ExpressionType.Parameter)
                {
                    ...
                    {
                        if (sqlWrapper.DatabaseTypeEnum is DatabaseTypeEnum.MySql or DatabaseTypeEnum.PostgreSql)
                            sqlWrapper += " IS TRUE";
                        else
                            sqlWrapper += " = 1";
                    }
        ...
        /// <returns>SqlWrapper</returns>
        public override SqlWrapper OrderBy(MemberExpression expression, SqlWrapper sqlWrapper, params OrderTypeEnum[] orders)
        {
            var tableAlias = string.Empty;
            var tableName = string.Empty;
            ...
            if (expression.Expression.NodeType == ExpressionType.Parameter)
            {
                sqlWrapper += tableAlias + sqlWrapper.GetColumnInfo(expression.Member.DeclaringType, expression.Member).ColumnName;
                if (orders?.Length > 0)
                    sqlWrapper += $" {(orders[0] == OrderTypeEnum.Descending ? "DESC" : "ASC")}";
            }
